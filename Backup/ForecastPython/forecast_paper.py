...
Modified
2019-12-16 -- pawitran -- Define function forcast_paper for automation process
...
class TSA(object):
    ...
    def EMA(self, dfData, dfIndices, isPredict):
        ...
        if not isPredict:
            ...
            # return prediction
            return self.ExpoSmth(dfData, dfIndices, True,
                                 EMA_ALPHA, EMA_BETA)
...
def forecast_paper():
    filename = 'D:/TPC_Forecast/02_Files/input_excel/historical_sales_paper.xlsx'
    file = pd.ExcelFile(filename)
    df = file.parse('Forecast_Input')
    df = pd.DataFrame(df)
    tsa = TSA(df)
    predictions = tsa.forecast()
    output_path = 'D:/TPC_Forecast/02_Files/output_csv/demandplan_paper.csv'
