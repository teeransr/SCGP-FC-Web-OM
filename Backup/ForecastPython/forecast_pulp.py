...
Modified
2019-12-16 -- pawitran -- Define function forcast_pulp for automation process
...
def forecast_pulp():
    filename = 'D:/TPC_Forecast/02_Files/input_excel/historical_sales_pulp.xlsx'
    file = pd.ExcelFile(filename)
    df = file.parse('Forecast_Input')
    df = pd.DataFrame(df)
    tsa = TSA(df)
    predictions = tsa.forecast()
    output_path = 'D:/TPC_Forecast/02_Files/output_csv/demandplan_pulp.csv'
